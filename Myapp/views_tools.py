...
# 进入正交工具页面
def sql_share(request):
    return render(request, 'welcome.html', {"whichHTML": "sql_share.html", "oid": request.user.id, **glodict(request)})


def add_sql(request):
    id = request.GET['id']
    sql_name = request.GET['sql_name']
    sql_des = request.GET['sql_des']
    Product = request.GET['Product']
    sql_content = request.GET['sql_content']
    DBs = request.GET['DBs']
    Product = DB_project.objects.filter(id=Product)[0]
    DBs = DB_database.objects.filter(id=DBs)[0]
    print(id)
    if id == "":
        DB_sql.objects.create(sql_name=sql_name, sql_des=sql_des,
                              Product=Product, sql_content=sql_content,
                              sql_db=DBs)
    if id != "":
        DB_sql.objects.filter(id=id).update(sql_name=sql_name, sql_des=sql_des,
                              Product=Product, sql_content=sql_content,
                              sql_db=DBs)
    return HttpResponse('')
...
def delete_sql(request):
    id = request.GET['id']
    DB_sql.objects.filter(id=id).delete()
    return HttpResponse('')


def run_sql(request):
    id = request.GET['id']
    sqls = DB_sql.objects.filter(id=id)[0]
    sql_content = sqls.sql_content
    sql_db = sqls.sql_db
    if sql_db.db_type == 'mysql':
        try:
            data = py_sql.py_mysql(sql_db, sql_content)
        except:
            data = '数据库配置有误，请检查后再执行！'
    else:
        data = '暂无此类型数据库！'
...
def get_sql(request):
    id = request.GET['id']
    sqls = DB_sql.objects.filter(id=id).values()[0]
    print(sqls)
    return HttpResponse(json.dumps(sqls), content_type='application/json')
